...
const canonical = "https://datamatt.io/attractions";
...
export const meta: MetaFunction = () => {
  return generateMeta({
    ...
    metaDescription:
      "Bored in Clemson? Not anymore! Here's a comprehensive list of attractions that are within driving distance of Clemson, South Carolina.",
    ...
    breadcrumbs: [
      ...
      {
        name: "Attractions In & Around Clemson, SC",
        item: "https://datamatt.io/attractions",
      },
    ],
  });
};
...
export async function loader({ context }: LoaderFunctionArgs) {
  const response = await dbLoader<AttractionItem>({
  ...
  const data = (await response.json()) as { items: AttractionItem[] };
